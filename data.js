(function (root, factory) {
  ...
  const controlKinds = new Set(["variable", "observation-window", "case-selector", "reveal"]);
  const control = (label, base, target, min, max, step, unit, kind) => {
    if (!controlKinds.has(kind)) throw new Error(`control.kind 必須明確指定，收到：${String(kind)}`);
    return { label, base, target, min, max, step, unit, kind };
  };
  ...
  const temples = [
    ...
    },
    {
      ...
      tracks:{
        foundation:[
          {code:"W-F1",title:"水之節奏",summary:"頻率變大時，波長變短、條紋變窄。",mission:"加快水眼的振動節奏，觀察池中的同心波紋會發生什麼變化。",skill:"頻率與波長趨勢",prerequisites:"讀圖、反比趨勢",time:"30–60 秒",image:"assets/ripple/water-1.webp",visual:"wave-frequency",known:["水波速度固定","單一波源","頻率由 2 Hz 調到 4 Hz"],control:control("頻率",2,4,1,5,.5,"Hz", "variable"),prediction:prediction("當你將水眼的振動節奏（頻率由 2 Hz 增加到 4 Hz）加快時，池面的同心波紋會怎麼改變？",[["narrow","變窄、變密"],["same","不變"],["wide","變寬、變疏"]]),reason:reason("哪一個因果鏈正確？",[["inverse","波速固定，f 增加所以 λ 減少"],["amplitude","頻率增加所以振幅必定增加"],["speed","頻率增加所以水波速度必定增加"]]),explanation:"在同一介質且波速固定時，v=fλ；頻率增加，波長縮短，所以條紋變密。",hint:"固定波速，把 f 與 λ 看成互相補償的兩個量。"},
          {code:"W-F2",title:"振幅幻象",summary:"振幅增大會加強起伏與對比，但不改變波長。",mission:"提高水眼振幅，分辨『看起來更強』與『條紋間距改變』。",skill:"區分振幅與波長",prerequisites:"波形判讀",time:"30–60 秒",image:"assets/ripple/water-2.webp",visual:"wave-amplitude",known:["頻率固定","水波速度固定","振幅由 1 級增至 3 級"],control:control("振幅",1,3,1,3,.5,"級", "variable"),prediction:prediction("提高水眼的起伏振幅後，池面的波紋主要會有什麼改變？",[["contrast","起伏與明暗對比變強"],["narrow","波長變短"],["faster","傳播速度必定增加"]]),reason:reason("為什麼條紋間距不變？",[["frequency","頻率與波速都沒改變"],["source","波源消失了"],["mass","水的質量變成零"]]),explanation:"振幅描述偏離平衡位置的程度；只改振幅不會改變由波速與頻率決定的波長。",hint:"把『波有多高』與『相鄰波峰隔多遠』分開看。"},
          {code:"W-F3",title:"雙源初醒",summary:"兩個同頻波源產生穩定的加強區與削弱區。",mission:"喚醒第二座水眼，觀察兩組波疊加後出現的新結構。",skill:"辨認干涉現象",prerequisites:"波的疊加概念",time:"45–75 秒",image:"assets/ripple/water-3.webp",visual:"wave-two-source",known:["兩波源頻率相同","兩波源持續振動","不計算相位差"],control:control("波源數量",1,2,1,2,1,"個", "variable"),prediction:prediction("喚醒第二座水眼，當兩座水眼以相同頻率振動時，水面會浮現什麼現象？",[["bands","穩定的加強與削弱區"],["nothing","完全沒有變化"],["stop","所有水波都消失"]]),reason:reason("這些區域形成的原因是什麼？",[["superpose","兩列波在各處疊加"],["gravity","重力忽然改變"],["boil","水溫升高"]]),explanation:"兩列同頻波重疊後，在不同位置持續加強或削弱，形成穩定干涉圖樣。",hint:"觀察哪些位置總是明顯起伏，哪些位置長期接近平靜。"},
          {code:"W-F4",title:"雙源之謎",summary:"波源間距增加時，節線與腹線分布通常變密。",mission:"頻率固定，只拉開兩座水眼，匹配更密集的干涉骨架。",skill:"波源間距與圖樣",prerequisites:"干涉現象",time:"45–90 秒",image:"assets/ripple/water-4.webp",visual:"wave-separation",known:["頻率固定","兩波源同頻","間距由 4 格增至 8 格"],control:control("波源間距",4,8,3,10,.5,"格", "variable"),prediction:prediction("當你將兩座水眼的距離拉得更開，交織出的節線與腹線分布會怎麼改變？",[["denser","變得較密"],["same","完全不變"],["none","全部消失"]]),reason:reason("同時有哪一項保持不變？",[["wavelength","單一波源的波長"],["geometry","干涉圖幾何"],["source-distance","波源距離"]]),explanation:"頻率與介質不變，所以單一波源波長不變；增加波源間距則讓可出現的程差範圍變大，干涉線數通常增加。",hint:"分開觀察『每一圈的間距』與『兩組圓相交形成的骨架』。"}
        ],
        advanced:[
  ...
  const prePlanNarratives = {
    ...
    "B-F1": ["兩根磁柱的朝向被失序打亂。", "距離固定，只能翻轉右側磁柱；先判斷兩根磁柱之間會怎樣，神門的開合由這個作用決定。", ["兩磁柱距離固定", "只改變右柱朝向", "其他力忽略不計"]],
